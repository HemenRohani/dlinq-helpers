...
namespace Kendo.DynamicLinq
{
    public static class QueryableExtensions
    {
            ...
            IEnumerable<Sort> sort, Filter filter, IEnumerable<Aggregator> aggregates, IEnumerable<Group> group)
        {
            ...
            {
                result.Group = GroupBy(queryable, pagedQuery, group);
            }
            else
            {
        ...
        private static IEnumerable<GroupResult> GroupBy<T>(IQueryable<T> queryable, IQueryable<T> pagedQuery, IEnumerable<Group> group)
        {

            var field = group.First().Field;

            var agg = group.First().Aggregates;
            var groupExpression = (Func<T, object>)DynamicExpression.ParseLambda(typeof(T), typeof(object), field).Compile();

            var pagedQueryGroups = pagedQuery.GroupBy(groupExpression);
            ...
            foreach (var item in pagedQueryGroups)
            {
                ...
                yield return new GroupResult
                {
                    ...
                    Items = item,
                    Field = field,
                    Aggregates = groupItems.AsQueryable().Aggregate(group.First().Aggregates)
                };
            }
        }
        ...
        private static object Aggregate<T>(this IQueryable<T> queryable, IEnumerable<Aggregator> aggregates)
        {
